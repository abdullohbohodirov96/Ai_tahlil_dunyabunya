...
export async function fetchLeadsFromSheet() {
  ...
  try {
    ...
    if (msg.includes("not found") || msg.includes("Unable to parse range")) {
      throw new Error(
        `Google Sheets xatosi: "${msg}". Tekshiring: (1) Sheet ID to'g'riligi, ` +
    ...
    if (msg.includes("permission") || msg.includes("PERMISSION_DENIED") || msg.includes("caller does not have")) {
      throw new Error(
        `Google Sheets xatosi: ruxsat yo'q. Sheetni service account emailiga ("${credentials.client_email}") ` +
        `Viewer sifatida ulashganingizga ishonch hosil qiling.`
      );
    }
    throw new Error(`Google Sheets xatosi: ${msg}`);
  }
}
